...
class Dialog extends CIQ.UI.BaseComponent {
	...
	getKeyboardSelectableItems() {
		return this.querySelectorAll(
			"[keyboard-selectable='true'], input, .ciq-select, cq-swatch, .ciq-btn, .ciq-btn-negative, cq-scroll li, ul cq-item"
		);
	}
...
class LookupDialog extends CIQ.UI.DialogContentTag {
	...
	selectItem(value) {
		...
		if (this.isComparison) {
			...
			changeSymbol({ symbol: symbolValue });
		}
		this.close();
	}
	...
	keyStroke(hub, key, e) {
		...
		switch (key) {
			...
			case "Enter":
				...
				if (active === this.input) this.form.requestSubmit();
				break;
 ...
 * - cq-max-height &ndash; Maximum height
 *
...
class Scroll extends CIQ.UI.BaseComponent {
	...
	resize() {
		...
		var reduceMenuHeight = this.reduceMenuHeight || 45;
		var contextHeight, contextTop;
		if (context) {
			var multiChartContainer = context.multiChartContainer;
			var contextRect = (
...
class Studies extends CIQ.UI.ContextTag {
	...
	renderMenu(stx) {
		...
		const contentNode = menu.find("cq-studies-content");
	...
	updateOrder() {
		...
		CIQ.Studies.Favorites.retrieveList(stx, (favoriteList) => {
			...
			const container = this.querySelector("cq-studies-content");
		...
		renderLegend() {
			...
			if (CIQ.Studies) {
				for (var id in stx.layout.studies) {
					...
					labelEl.html(sd.inputs.display);
		...
		setPanelLegendWidth() {
			...
			const panelControl = this.node.parents("div.stx-panel-control")[0],
				...
				lastBtnOffset = lastBtn
					? `${lastBtn.offsetLeft + lastBtn.offsetWidth}px`
					: null,
				// Add or remove a class to the panel buttons that gives them a background.
				setPanelBtnBg = (doesOverlapLegend) => {
					panelControl
						.querySelectorAll(".stx-btn-panel.stx-show")
						.forEach((btn) =>
							btn.firstChild.classList[doesOverlapLegend ? "remove" : "add"](
								"stx-ico-bg"
							)
						);
				};
			// If all the panel buttons overlap with the panel legend, we add or expand a <span> element, so the legend background covers the buttons. Otherwise, we add a background to the buttons.
			if (firstBtnOffset && firstBtnOffset <= 208) {
				setPanelBtnBg(true);
				if (
					!this.nextSibling ||
					!this.nextSibling.classList.contains("stx-btn-bg")
				) {
					this.insertAdjacentHTML(
						"afterend",
						`<span class="stx-btn-bg" style="width: ${lastBtnOffset};">`
					);
				} else {
					this.nextSibling.style = `width: ${lastBtnOffset}`;
				}
			} else {
				setPanelBtnBg(false);
			}
...
let __js_webcomponents_studyMenuManager_ = (_exports) => {
...
class Undo extends CIQ.UI.ContextTag {
	...
	setContext(context) {
		...
		if (this.context.topNode.getCharts) {
			let stxArr = this.context.topNode.getCharts();
...
export {__js_webcomponents_studyMenuManager_ as studyMenuManager};
...
if (typeof __TREE_SHAKE__ === "undefined" || !__TREE_SHAKE__) {
	_exports.CIQ.activateImports(
		...
		__js_webcomponents_studyMenuManager_,
