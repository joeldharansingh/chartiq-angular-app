...
class BaseComponent extends HTMLElement {
	...
	addDefaultMarkup(component, markup) {
		...
		if (this.shadowRoot)
			[...node.children].forEach((child) => this.shadowRoot.append(child));
	}
	/**
	 * Emits a custom event from the webcomponent.
	 *
	 * @param {object} params
	 * @param {HTMLElement} [params.emitter] The element that will be emitting the event.  Required if not provided in `detail`.
	 * @param {string} [params.actualType] The event type that represents the user's action.  Required if not provided in `detail`.
	 * @param {Object} [params.listenType="click"] The event type to dispatch to a listener on the component.
	 * @param {Object} [params.detail={}] Optional extra parameters to include in the detail of the Custom Event.
	 *
	 * @alias emitCustomEvent
	 * @memberof CIQ.UI.BaseComponent.prototype
	 * @since 8.9.0
	 */
	emitCustomEvent({ emitter, actualType, listenType = "click", detail = {} }) {
		emitter.dispatchEvent(
			new CustomEvent(listenType, {
				bubbles: true,
				cancelable: true,
				composed: true,
				detail: CIQ.extend({ emitter, actualType }, detail)
			})
		);
	}
	/**
	...
	qsa(path, root, bustShadowRoots) {
		...
		findItemContainer(root || document);
		return matches;
	}
	/**
	 ...
	 *
	 * @param {ShadowRoot} root The shadow root to set up.
	 *
	 * @alias setupShadow
	 * @memberof CIQ.UI.BaseComponent.prototype
	 */
	setupShadow(root) {
		const self = this;
		self.setAttribute("interactable", "");
			...
			[...stylesheets].reverse().forEach((stylesheet) => {
				if (stylesheet.tagName === "LINK") {
					...
					lnk.onload = () => {
						...
						if (self.refresh) setTimeout(self.refresh.bind(self));
					};
		...
		copyStylesheets(
			self.ownerDocument.head.querySelectorAll(
				"link:not([no-wc-import]), style:not([no-wc-import])"
			)
		);
		if (!self.copyStylesheetListener)
			self.copyStylesheetListener = self.ownerDocument.head.addEventListener(
				"stylesheetload",
				(e) => {
					copyStylesheets([e.detail.link]);
				}
			);
		const closestContextContainer = CIQ.UI.closestContextContainer(self);
		CIQ.UI.shadowComponents.set(self, closestContextContainer);
		self
			.getAttributeNames()
			.filter((attribute) => /-feature$/.test(attribute))
			.forEach((attribute) => self.removeAttribute(attribute));
		...
		if (closestContextContainer.currentTheme)
			self.classList.add(closestContextContainer.currentTheme);
		if (closestContextContainer.classList.contains("full-screen"))
			self.classList.add("full-screen");
	}
	/**
	 ...
	 */
	removeFocused(items) {
		if (!items) items = this.qsa("[cq-focused]", this, true);
		items.forEach((item) => {
...
class ContextTag extends BaseComponent {
	...
	setContextHolder() {
		...
		if (!nearestContext) {
			...
			if (dialog) nearestContext = dialog.firstElementChild.context;
		}
		if (!nearestContext) {
...
customElements.define("cq-marker", ContextTag);
/**
...
DrawingEdit.prototype.edit = function () {
	if (this.drawingContext) {
		...
		if (
			settingsPalette &&
			!/break.*sm\b/.test(this.context.topNode.className)
		) {
...
CIQ.UI.Layout.prototype.setAggregationEdit = function (obj, field) {
	...
	if (field === "auto") {
		...
		completeAggEdit();
		const parent = obj.params.parent;
		if (!parent) return;
		Array.from(
		...
		tuple.obj[tuple.member] = obj.node.value;
		completeAggEdit();
	}
};
/**
...
CIQ.UI.Layout.prototype.chartType = function (node) {
	...
	const iconNode = node.qsa(".icon", root, true)[0];
	const labelNode = node.qsa("[label]", root, true)[0];
	const tooltip = node.qsa("[cq-tooltip]", root, true)[0];
	const listener = ({ obj: { chartType, aggregationType } }) => {
		...
		const typeToTranslate = type
			.split(" ")
			.map((t) => CIQ.capitalize(t))
			.join(" ");
		...
		if (!labelNode && !iconNode) {
			const labelNode = node
				.closest("cq-menu")
				.querySelector(`*[data-value='${type}'] *[ciq-label]`);
...
CIQ.UI.Layout.prototype.getLanguage = function (node) {
	...
	const listener = (obj) => {
		...
		if (lang) {
			...
			const flag = node.querySelector("cq-flag");
...
CIQ.UI.KeystrokeHub.prototype.handler = function (obj) {
	...
	const targetTagName = e.composedPath()[0].tagName;
	switch (key) {
		case "Shift":
...
class UIManager extends HTMLElement {
	...
	openMenu(menu, params) {
		...
		setTimeout(function () {
			var input = menu.querySelector("input:first-child");
...
class Chart {
	...
	createChartAndUI({ container, config }) {
		...
		function addPluginContainer(uiContext) {
			...
			uiContext.topNode.appendChild(pluginContainer);
		...
		if (config.language) CIQ.I18N.localize(stx, config.language);
	...
	getCommonUI(container) {
		return getElements(".ciq-nav, .ciq-footer", container).filter((el) => {
			return (
				...
				!el.matches("cq-toggle[cq-member=tfc]")
			);
		});
...
CIQ.UI.setDefaultStates = function (context) {
	...
	if (selectorsToEnable && selectorsToEnable.length) {
		...
		context.topNode.setAttribute("aria-hidden", true);
	}
