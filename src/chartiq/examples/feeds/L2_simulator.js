...
CIQ.simulateL2 = function (params) {
	function moveBidAsk(close) {
		...
		for (i = 0; i < bidVs.length; i++) {
			bidVs[i] = Math.max(0, bidVs[i] + Math.round(10 * CIQ.random() - 5));
		}
		for (i = 0; i < askVs.length; i++) {
			askVs[i] = Math.max(0, askVs[i] + Math.round(10 * CIQ.random() - 5));
		}
		for (i = 0; i < bidVs.length; i++) {
			if (bidVs[i]) {
				data.Bid = bids[i];
	...
	function onTrade(appendQuotes, chart, params) {
		...
		for (var i = 0; i < appendQuotes.length; i++) {
			...
			if (stx.chart.market.isOpen()) {
				appendQuotes[i].LastSize = Math.round(CIQ.random() * 100);
				if (
	...
	function onInterval(stx) {
		return function () {
			...
			var randomClose = 100 + Math.round(10 * CIQ.random() - 5) * 0.01;
			var data = moveBidAsk.call(stx, close || randomClose);
			if (close === null) {
				data.Last = randomClose;
				data.LastSize = Math.round(CIQ.random() * 100);
			}
			data.DT = new Date();
